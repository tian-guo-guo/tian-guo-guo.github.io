...
关于二者的区别可以分为两大类进行比较。一个是功能上的区别，另一个是书写习惯的区别。
关于功能上的差别主要是XHTML可兼容各大浏览器，手机设备等，并且浏览器也能快速正确的编译网页。
关于书写习惯的差别，XHTML的语法书写要更加严格一些。所有标签都必须小写，标签必须成对，出现标签顺序必须正确。所有属性都必须使用双引号。

### div+css布局有什么优点？
我们大致能总结说明了以下4点。
首先第一点，div+css布局可以实现HTML结构和css的表现分离，这样可以使得HTML的文档了更加精简清晰，可读性高，加载的速度也会更快。
第二点，想要使html拥有不同的呈现，实现会更加方便，因为我们只需要引入不同的CSS文档就可以了。这样的提高了我们整个文档的可复用性和设计的一个灵活性。
第三点，浏览器会缓存静态文件，比如说可以缓存CSS文件，这样就有助于提高我们网站的性能和SEO的评估。
第四点，因为实在了结构和表现分明，所以整个代码在上线之后可维护性会更好。
